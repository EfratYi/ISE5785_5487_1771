...
public class Minip2 {
     ...
     */
    @Test
    void createSceneWithMultipleGeometriesSoftShadowsBVH() {
        Scene scene = new Scene("ComplexSceneWithSoftShadowsBVH");
        ...
        cameraBuilder
                ...
                .build()
                .renderImage()
                .writeToImage("multiply_amount_of_geometries_with_soft_shadows_with_BVH");
    }
    ...
    void createSceneWithMultipleGeometriesSoftShadows_MultiThreaded() {
        Scene scene = new Scene("Scene with many geometries and soft shadows MultiThreaded");
     ...
     */
    @Test
    void createSceneWithMultipleGeometriesSoftShadows_MultiThreaded_BVH() {
        Scene scene = new Scene("Scene with many geometries and soft shadows MultiThreaded BVH");
        ...
        cameraBuilder
                ...
                .setMultithreading(-2)
                .setDebugPrint(1.0)
                .enableBVH(scene) // Enable BVH for performance
                .build()
                .renderImage()
                .writeToImage("multiply_amount_of_geometries_with_soft_shadows_with_window_multythreaded_BVH");
     }
     ...
     */
    @Test
    void createSceneWithMultipleGeometriesSoftShadows_MultiThreaded_CBR() {
        Scene scene = new Scene("Scene with many geometries and soft shadows MultiThreaded CBR");
        ...
        cameraBuilder
                ...
                .setMultithreading(-2)
                .setDebugPrint(1.0)
                .enableCBR(scene) // Enable CBR for performance
                .build()
                .renderImage()
                .writeToImage("multiply_amount_of_geometries_with_soft_shadows_with_window_multythreaded_CBR");
    }

    /**
     * Creates a realistic floor with a matte material.
     *
    ...
    void createSceneWithBVHManual() {
        ...
        Geometries wallsGroup = new Geometries();
        wallsGroup.add(createTwoWallsWithWindows());

        Geometries floorGroup = new Geometries();
        floorGroup.add(createRealisticFloor(-60, 400));

        Geometries mirrorGroup = new Geometries();
        mirrorGroup.add(createLargeMirror());

        Geometries mainPyramidGroup = new Geometries();
        mainPyramidGroup.add(createEnhancedPyramid());

        Geometries pyramidsGroup = new Geometries();
        for (int i = 0; i < 10; i++) {
            double x = -140 + (i % 5) * 70;
            double z = -100 + (i / 5) * 80;
            Point center = new Point(x, -60, z);
            pyramidsGroup.add(createRandomPyramid(center, 30, 40,
                    new Color(30 + i * 10, 100 + (i * 5) % 100, 60 + (i * 7) % 150)));
        }

        Geometries spheresGroup = new Geometries();
        Random rand = new Random();
        for (int i = 0; i < 10; i++) {
            double x = -120 + rand.nextDouble() * 240;
            double z = 120 + rand.nextDouble() * 100;
            ...
            Color color = new Color(
                    ...
                    50 + rand.nextInt(100),
                    80 + rand.nextInt(100));
            spheresGroup.add(createSphere(center, radius, color));
        }

        Geometries cubesGroup = new Geometries();
        for (int i = 0; i < 5; i++) {
            double x = -80 + i * 40;
            double y = -45;
            double z = 150;
            Point center = new Point(x, y, z);
            Color color = new Color(100 + i * 20, 80 + i * 10, 150 - i * 10);
            cubesGroup.add(createTranslucentCube(center, 25, color));
        }

        Geometries glassGroup = new Geometries();
        glassGroup.add(createGlassInWindow());

        Geometries rootGroup = new Geometries();
        rootGroup.add(floorGroup);
        rootGroup.add(wallsGroup);
        rootGroup.add(mirrorGroup);
        rootGroup.add(mainPyramidGroup);
        rootGroup.add(pyramidsGroup);
        rootGroup.add(spheresGroup);
        rootGroup.add(cubesGroup);
        rootGroup.add(glassGroup);

        scene.geometries.add(rootGroup);

        scene.lights.add(new SpotLight(
                new Color(1400, 1100, 1000),
                new Point(200, 20, -45),
                new Vector(0.9995, 0, 0))
                .setRadius(12).setKc(1).setKl(0.0001).setKq(0.000001));
        ...
        scene.lights.add(new PointLight(
                new Color(80, 70, 65),
                new Point(-80, 60, 100))
                .setRadius(8).setKc(1).setKl(0.0002).setKq(0.000002));

        scene.lights.add(new SpotLight(
                new Color(100, 80, 50),
                new Point(150, 30, 50),
                new Vector(-1, -0.5, -1))
                .setRadius(6).setKc(1).setKl(0.0003).setKq(0.000003));
